...
static int nr_pf_dl(module_id_t module_id,   //采用比例公平算法来调度UE #############这里调度的是每一个slice中的UE。 Proportional fair scheduling algorithm
                     frame_t frame,
                     sub_frame_t slot,
                     NR_UE_info_t **UE_list,
                     int max_num_ue,
                     int n_rb_sched,         //当前可用于调度的资源块数量
                     uint16_t *rballoc_mask,  //用于跟踪哪些资源块已被分配的位掩码
                     void *data)             //额外的调度相关数据
{
  ...
  UE_iterator(UE_list, UE) {
    //i++;
    //printf("the number of UE_LIST %d", i);
    if (UE->Msg4_ACKed != true)   //是否已确认msg4
      continue;

    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
    NR_UE_DL_BWP_t *current_BWP = &UE->current_DL_BWP;

    if (sched_ctrl->ul_failure)  //是否存在上行链路故障
      continue;
...
//如果不能分配资源块，记录日志并跳过当前UE
      ...
      /* Calculate coeff */      //基于UE的即时吞吐量和计算出的TBS
      ...
      float coeff_ue = (float) tbs / UE->dl_thr_ue;    //基于UE的即时吞吐量和计算出的TBS
      ...
      UE_sched[curUE].UE=UE; //将每个UE的调度度量和其他信息填入UE_sched数组
      curUE++;
    }
  }

//使用qsort对UE_sched数组根据coeff进行排序
...
//尝试为下行控制信息指示（DCI）找到物理上行控制信道（PUCCH）
    int r_pucch = nr_get_pucch_resource(sched_ctrl->coreset, ul_bwp->pucch_Config, CCEIndex);
...
//根据时域分配信息生成一个位图，表示PDSCH的起始符号和符号数量
    const uint16_t slbitmap = SL_to_bitmap(tda_info->startSymbolIndex, tda_info->nrOfSymbols);

    int rbStop = 0;
    int rbStart = 0;

    get_start_stop_allocation(mac, iterator->UE, &rbStart, &rbStop); //计算当前UE可用的起始和结束资源块位置
    // Freq-demain allocation
    while (rbStart < rbStop && (rballoc_mask[rbStart] & slbitmap) != slbitmap)
      rbStart++;

    uint16_t max_rbSize = 0;

    while (rbStart + max_rbSize <= rbStop && (rballoc_mask[rbStart + max_rbSize] & slbitmap) == slbitmap && max_rbSize < n_rb_sched)
      max_rbSize++;
    //##################yph, here for max_rbsize, without slice, it will always plus plus, so I add && max_rbSize < n_rb_sched

    sched_pdsch->dmrs_parms = get_dl_dmrs_params(scc, 
                                                 dl_bwp,
                                                 tda_info,
                                                 sched_pdsch->nrOfLayers);
    sched_pdsch->Qm = nr_get_Qm_dl(sched_pdsch->mcs, dl_bwp->mcsTableIdx); //调制阶数
    sched_pdsch->R = nr_get_code_rate_dl(sched_pdsch->mcs, dl_bwp->mcsTableIdx);  //码率
    ...
    //计算所需的资源块数和传输块大小，基于当前的信道质量和数据需求
...
nr_dl_sched_algo_t nr_proportional_fair_wbcqi_dl = {   //公平调度算法的结构体
  .name  = "nr_proportional_fair_wbcqi_dl",            //用户级调度器的算法也可以针对每个切片进行定制
...
static void nr_fr1_dlsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)   //预处理器中的切片级调度程序
{
  ...
  int bw = scc->downlinkConfigCommon->frequencyInfoDL->scs_SpecificCarrierList.list.array[0]->carrierBandwidth;//数值是106
  ...
  /* proportional fair scheduling algorithm */   /* #################### 这里是算法的开始 ##################*/
  RC.nrmac[module_id]->pre_processor_dl.dl_algo.run(module_id,   //默认情况下   这里的run是空函数指针 定义为 nr_pf_dl
                                                    frame,
                                                    slot,
                                                    UE_info->list,
                                                    max_sched_ues,
...
nr_pp_impl_param_dl_t nr_init_fr1_dlsch_preprocessor(int CC_id) {
  ...
  impl.dl_algo = nr_proportional_fair_wbcqi_dl;   //用户级调度器的算法也可以针对每个切片进行定制
                         ...
                         nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  /* #########################################PREPROCESSOR######################################## */
  pthread_mutex_lock(&gNB_mac->UE_info.mutex); //多线程
  gNB_mac->pre_processor_dl.dl(module_id, frame, slot);  //预处理器中的切片级调度程序 
   //高宇：可以把dl赋值成任意一个这种类型的函数，例如nvs_nr_dl，也可以是自己实现的同类型的函数
  pthread_mutex_unlock(&gNB_mac->UE_info.mutex);
//**************UE.sched_pdsch中的rbstart, rbsize, TBS都改变了
  ...
  UE_iterator(UE_info->list, UE) {   /* ############# 遍历所有的UE ############## */
    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
    NR_UE_DL_BWP_t *current_BWP = &UE->current_DL_BWP;
    ...
    if (frame == (sched_ctrl->ta_frame + 10) % 1024) {  //时间对齐
    ...
    if (!pdcch_pdu) {
      ...
      nr_configure_pdcch(pdcch_pdu, coreset, &sched_ctrl->sched_pdcch);//填充和UE控制相关的参数，通过nfapi给物理层，包括BWPsize and BWPstart
